...
def test_device_list_view(admin_client, admin_user, django_assert_max_num_queries):
    with override_settings(WAGTAIL_2FA_REQUIRED=True):
        admin_device = TOTPDevice.objects.create(
            name="Initial", user=admin_user, confirmed=True
        )
        ...
        with django_assert_max_num_queries(14):
            response = admin_client.get(
                reverse("wagtail_2fa_device_list", kwargs={"user_id": admin_user.id})
            )
...
def test_delete_user_device_as_admin(client, admin_user, user, monkeypatch):
    with override_settings(WAGTAIL_2FA_REQUIRED=True):
        ...
        response = client.post(endpoint, {"user_id": user.id})
...
def test_delete_user_device_as_admin_unverified(admin_client, user, monkeypatch):
    with override_settings(WAGTAIL_2FA_REQUIRED=True):
        ...
        endpoint = reverse("wagtail_2fa_device_remove", kwargs={"pk": device.id})
        response = admin_client.post(endpoint, {"user_id": user.id})
        assert response.status_code == 302
...
class TestViewsChangeUserPermission:
    ...
    def test_device_list_view_for_other_user_raises_error(
        ...
        with override_settings(WAGTAIL_2FA_REQUIRED=True):
            ...
            with pytest.raises(PermissionDenied):
                DeviceListView.as_view()(request, user_id=user.id)

    def test_device_delete_view_for_own_user_returns_200(self, verified_user, rf):
        with override_settings(WAGTAIL_2FA_REQUIRED=True):
            device = TOTPDevice.objects.devices_for_user(
                verified_user, confirmed=True
            ).first()
            request = rf.get("foo")
            request.user = verified_user
    ...
    def test_device_delete_view_for_other_user_raises_error(
        self, user, verified_user, rf
    ):
        with override_settings(WAGTAIL_2FA_REQUIRED=True):
            ...
            TOTPDevice.objects.devices_for_user(verified_user, confirmed=True).first()
            request = rf.get("foo")
            request.user = verified_user
    ...
    def test_device_update_view_for_other_user_raises_error(
        self, user, verified_user, rf
    ):
        with override_settings(WAGTAIL_2FA_REQUIRED=True):
            other_device = TOTPDevice.objects.create(
            ...
            with pytest.raises(Http404):
                DeviceUpdateView.as_view()(request, pk=other_device.id)
